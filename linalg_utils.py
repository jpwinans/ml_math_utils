...
def get_index_first_non_zero_value_from_column(M, column, starting_row):
    """
    Retrieve the index of the first non-zero value in a specified column of
    the given matrix.
    ...
    Returns:
    int: The index of the first non-zero value in the specified column,
...
def get_index_first_non_zero_value_from_row(M, row, augmented=False):
    """
    Find the index of the first non-zero value in the specified row of the
    given matrix.

    Parameters:
    - M (numpy.array): The input matrix to search for non-zero values.
    - row (int): The index of the row to search.
    - augmented (bool): Pass this True if you are dealing with an augmented
        matrix, so it will ignore the constant values (the last column
...
def row_echelon_form(A, B):
    ...
    # Before any computation, check if matrix A (coefficient matrix) has non-
    # zero determinant.
    ...
    for row in range(num_rows):
        ...
        # The diagonal elements in a matrix has the same index for row and
        # column.
        pivot_candidate = M[row, row]

        # If pivot_candidate is zero, it cannot be a pivot for this row.
        # So the first step is to look at the rows below it to check if there
        # is a non-zero element in the same column.
        ...
        # If pivot_candidate is already non-zero, then it is the pivot for
        # this row
        else:
        ...
        # Divide the current row by the pivot, so the new pivot will be 1. Use
        # formula current_row -> 1/pivot * current_row
        ...
        for j in range(row + 1, num_rows):
            ...
            # The values in row j that are below the pivot must have column
            # index the same index as the column index for the pivot.
...
def back_substitution(M):
    """
    Perform back substitution on an augmented matrix (with unique solution) in
    reduced row echelon form to find the solution to the linear system.

    Parameters:
    - M (numpy.array): The augmented matrix in row echelon form with unitary
        pivots (n x n+1).
    ...
    for row in reversed(range(num_rows)):
        ...
        for j in reversed(range(row)):
            ...
            # Get the value of the element at the found index in the row to
            # reduce
            value = row_to_reduce[index]

            # Perform the back substitution step using the formula
...
def gaussian_elimination(A, B):
    """
    Solve a linear system represented by an augmented matrix using the
    Gaussian elimination method.

    Parameters:
    - A (numpy.array): Square matrix of size n x n representing the
        coefficients of the linear system
    - B (numpy.array): Column matrix of size 1 x n representing the constant
        terms.

    Returns:
    numpy.array or str: The solution vector if a unique solution exists, or a
        string indicating the type of solution.
    """

    # Get the matrix in row echelon form
    row_echelon_M = row_echelon_form(A, B)

    # If the system is non-singular, then perform back substitution to get the
    # result.
...
def string_to_augmented_matrix(equations):
    ...
    for equation in equation_list:
        ...
        coefficients.append([left_side.coeff(variable) for variable in
                             variables])
